...
public class MultiPortableDialog extends JDialog {
	...
	public MultiPortableDialog(JDomNodeAdapter dsNode, PortableType type) {
		this.type = type;
		if(type.equals(PortableType.IMPORTABLE))
			portableType = Resources.get("IMPORTABLE");
		else
			portableType = Resources.get("EXPORTABLE");
		this.node = dsNode;
		this.init();
		this.setModal(true);
		this.pack();
	...
	private void createPortableName() {
		setSelectedPortable();
		MultiPortableDialog.this.setVisible(false);
		LinkItemsDialog linkDialog = new LinkItemsDialog(this.type,"");
...
//			conditions.put(Resources.get("GUI"), McGuiUtils.INSTANCE.getGuiType().toString());
    		...
    		for(Element containerE: containerElements) {
    			if(type.equals(PortableType.IMPORTABLE)) {
    				selList.addAll(JDomUtils.searchElementList(containerE, Resources.get("FILTER"), null));    				
    			}
    			else {
